    ...
    <DashboardLayout title="DV Incident Log" subtitle="Document incidents, medical reports & evidence" backHref="/dashboard">
      <div className="w-full max-w-5xl mx-auto space-y-6">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-3">
            <div className="rounded-lg border border-red-200/30 bg-red-50/50 p-3 sm:p-4 shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-wide text-red-600">Incidents Logged</p>
              <p className="mt-1 text-lg sm:text-xl font-bold text-red-700">{summary?.totalIncidents ?? 0}</p>
            </div>
            <div className="rounded-lg border border-blue-200/30 bg-blue-50/50 p-3 sm:p-4 shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-wide text-blue-600">Medical Reports</p>
              <p className="mt-1 text-lg sm:text-xl font-bold text-blue-700">{summary?.totalMedicalReports ?? 0}</p>
            </div>
            <div className="col-span-2 lg:col-span-1 rounded-lg border border-gray-200/30 bg-white/50 p-3 sm:p-4 shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Incident Types</p>
              <p className="mt-1 text-lg sm:text-xl font-bold text-gray-900">{Object.keys(summary?.byType ?? {}).length}</p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <button
            ...
            className="group flex items-center gap-4 rounded-xl border-2 border-red-200 bg-gradient-to-br from-red-50 to-red-100/50 p-4 sm:p-5 shadow-sm hover:shadow-md hover:border-red-300 transition-all text-left"
          >
            ...
            <div>
              <p className="font-semibold text-gray-900">Log Incident</p>
              <p className="text-xs text-gray-500 mt-0.5">Document what happened</p>
            </div>
          </button>
          ...
          >
            ...
            <div>
              <p className="font-semibold text-gray-900">Incident Timeline</p>
              <p className="text-xs text-gray-500 mt-0.5">View all documented incidents</p>
            </div>
          </button>
          ...
          >
            ...
            <div>
              <p className="font-semibold text-gray-900">Medical Reports</p>
              <p className="text-xs text-gray-500 mt-0.5">Organize medical documentation</p>
            </div>
          </button>
        </div>
        ...
        {!loading && summary && totalByType > 0 && (
          <div className="rounded-lg border border-gray-200/20 bg-white/50 p-4 sm:p-6 shadow-sm">
            <h3 className="mb-4 text-base font-semibold text-gray-900">Incidents by Type</h3>
        ...
        {!loading && summary && summary.totalIncidents === 0 && (
          <div className="rounded-xl border border-dashed border-gray-300 bg-white/40 p-8 sm:p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-700">No incidents logged</h3>
            <p className="mt-1 text-sm text-gray-500">If you're experiencing domestic violence, document every incident here.</p>
            <button
              onClick={() => navigate('/dashboard/dv-log/add')}
        ...
        <div className="rounded-lg border border-red-200 bg-red-50/50 p-4 sm:p-5">
          <h3 className="mb-2 font-semibold text-red-900">Important Safety Information</h3>
