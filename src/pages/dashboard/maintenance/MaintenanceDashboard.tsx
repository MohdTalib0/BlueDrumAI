...
export default function MaintenanceDashboard() {
  ...
  const loadSummary = async (signal?: AbortSignal) => {
    try {
      ...
      const res = await fetch(`${getEdgeFunctionUrl('maintenance')}/summary`, {
    ...
    <DashboardLayout title="Maintenance Calculator" subtitle="Calculate & document your maintenance rights" backHref="/dashboard">
      <div className="w-full max-w-5xl mx-auto space-y-6">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
            <div className="rounded-lg border border-teal-200/20 bg-teal-50/50 p-3 sm:p-4 shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-wide text-teal-600">Latest Maintenance</p>
              <p className="mt-1 text-lg sm:text-xl font-bold text-teal-700">
            ...
            <div className="rounded-lg border border-gray-200/20 bg-white/50 p-3 sm:p-4 shadow-sm">
              ...
              <p className="mt-1 text-lg sm:text-xl font-bold text-gray-900">{summary?.totalCalculations ?? 0}</p>
              <p className="text-[10px] sm:text-xs text-gray-500">saved scenarios</p>
            </div>
            <div className="rounded-lg border border-rose-200/20 bg-rose-50/50 p-3 sm:p-4 shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-wide text-rose-600">Monthly Expenses</p>
              <p className="mt-1 text-lg sm:text-xl font-bold text-rose-700">{formatCurrency(summary?.totalMonthlyExpenses ?? 0)}</p>
              <p className="text-[10px] sm:text-xs text-gray-500">documented</p>
            </div>
            <div className="rounded-lg border border-blue-200/20 bg-blue-50/50 p-3 sm:p-4 shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-wide text-blue-600">Expense Items</p>
              <p className="mt-1 text-lg sm:text-xl font-bold text-blue-700">{summary?.totalExpenses ?? 0}</p>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <button
            ...
            className="group flex items-center gap-4 rounded-xl border-2 border-teal-200 bg-gradient-to-br from-teal-50 to-teal-100/50 p-4 sm:p-5 shadow-sm hover:shadow-md hover:border-teal-300 transition-all text-left"
          >
            ...
            <div>
              <p className="font-semibold text-gray-900">Calculate Maintenance</p>
              <p className="text-xs text-gray-500 mt-0.5">Based on husband's income</p>
            </div>
          </button>
          ...
          >
            ...
            <div>
              <p className="font-semibold text-gray-900">Log Expenses</p>
              <p className="text-xs text-gray-500 mt-0.5">Education, medical & more</p>
            </div>
          </button>
          ...
          >
            ...
            <div>
              <p className="font-semibold text-gray-900">Know Your Rights</p>
              <p className="text-xs text-gray-500 mt-0.5">Legal provisions & case law</p>
            </div>
          </button>
        </div>
        ...
        {!loading && summary && summary.totalCalculations === 0 && summary.totalExpenses === 0 && (
          <div className="rounded-xl border border-dashed border-gray-300 bg-white/40 p-8 sm:p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-700">No calculations yet</h3>
            <p className="mt-1 text-sm text-gray-500">Calculate your maintenance entitlement based on husband's income.</p>
            <button
              onClick={() => navigate('/dashboard/maintenance/calculate')}
        ...
        <div className="rounded-lg border border-teal-200 bg-teal-50/50 p-4 sm:p-5">
          <h3 className="mb-2 font-semibold text-teal-900">About Maintenance Calculator</h3>
